...
Таблица `cats` содержит список категорий товаров, включающих два поля: `name` - наименование категории, и `cat`- условный порядковый номер категории (первичный ключ таблицы `cats`). Заполнение таблицы происходит при создании схемы БД.
...
* `total_received_on_dates.sql` - количество принятых товаров (их цен) по датам;
...
* `price_changes_for_all_prods.sql` - приращения цен по всем товарам на две фиксированные даты. Строки, для которых изменение цены составило более 900% или менее -95%, считаются ошибочными и в результаты запроса не включаются.
...
* `price_changes_in_cats_on_date.sql` - запрос, который использует предыдущую функцию для расчета среднего прироста цен (за месяц) по каждой категории в отдельности (на заданную дату).
